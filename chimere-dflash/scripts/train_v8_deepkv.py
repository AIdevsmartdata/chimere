#!/usr/bin/env python3
"""
train_v8_deepkv.py — Train DFlash v8.1 drafter with Deep KV Injection.

Key changes from train_v8.py:
  - Uses DFlashDraftModelV8 (deep KV injection per layer)
  - Output dir: checkpoints_v8_deepkv
  - Version string: v8_deepkv

Usage:
  python scripts/train_v8_deepkv.py \
...
# TF32 for Blackwell/Ampere+ — ~2× matmul throughput, 19-bit mantissa (sufficient for training)
torch.backends.cuda.matmul.allow_tf32 = True
torch.backends.cudnn.allow_tf32 = True
torch.backends.cudnn.benchmark = True  # auto-select fastest cuDNN kernels (+3%)
...
def train_epoch(model, dataloader, optimizer, scheduler, scaler, device, use_amp,
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Use .detach() instead of .item() to avoid CPU-GPU sync every step (+5%)
        cur_loss = loss.detach() * grad_accum_steps
        step_loss += cur_loss
        total_loss += cur_loss
        ...
        if (batch_idx + 1) % log_every == 0:
            ...
            avg_step = (step_loss / log_every).item()  # sync only for logging
    ...
    avg_loss = (total_loss / max(1, n_steps)).item()  # single sync at epoch end
    return avg_loss, total_tokens


@torch.no_grad()
def evaluate(model, dataloader, device, use_amp, chunk_size):
    """Evaluate with single forward — chunked accuracy inside the same pass."""
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_count = 0
    ...
    for batch in dataloader:
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=use_amp):
            # Single forward: use forward_train which returns logits for accuracy
            loss, logits, _ = model.forward_train(
                block_ids, ctx_hidden, ctx_lengths,
                anchor_positions=anchor_positions,
            )
...
def main():
    parser = argparse.ArgumentParser(description="Train DFlash v8.1 (Deep KV Injection + Multi-Anchor)")
    parser.add_argument("--features-dir", type=str, default="data/features_fullseq")
    parser.add_argument("--output-dir", type=str, default="checkpoints_v8_deepkv")
    ...
    print("=" * 60)
    print(" DFlash v8.1 — Deep KV Injection + Multi-Anchor")
    print("=" * 60)
    ...
    # With preload, data is already in RAM — no need for workers (avoids 17GB fork overhead)
    n_workers = 0 if args.preload else 4
    n_val_workers = 0 if args.preload else 2
    train_loader = DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=True,
        collate_fn=collate_fn, num_workers=n_workers, pin_memory=(not args.preload),
        drop_last=True,
    )
    val_loader = DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False,
        collate_fn=collate_fn, num_workers=n_val_workers, pin_memory=(not args.preload),
    )

    # Create model (V8 with deep KV injection)
    ...
    # Resume BEFORE compile (compiled model wraps keys with _orig_mod.)
    # Load to CPU to avoid doubling GPU memory during state_dict assignment
    start_epoch = 0
    if args.resume:
    ...
    # NOTE: Skip optimizer state restore to avoid OOM (optimizer state ~4 GB + model ~4 GB
    # exceeds VRAM before first forward). Optimizer reinitializes lazily. Minor quality impact.
    # if args.resume and "optimizer_state_dict" in ckpt:
    #     optimizer.load_state_dict(ckpt["optimizer_state_dict"])

    # Recompute schedule for remaining epochs only
    remaining_epochs = args.epochs - (start_epoch if args.resume else 0)
    num_training_steps = len(train_loader) // args.grad_accum * remaining_epochs
    num_warmup_steps = int(len(train_loader) // args.grad_accum * 0.02)  # brief warmup on resume
    if not args.resume:
        num_training_steps = len(train_loader) // args.grad_accum * args.epochs
        num_warmup_steps = int(num_training_steps * args.warmup_ratio)
    scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps)

    if False and args.resume and "scheduler_state_dict" in ckpt:
