...
The project went through 4 architectural pivots over 4 weeks, each yielding a specific insight:

\begin{description}
  \item[v1 (Gaussian $\to$ Discrete):] Continuous Gaussian diffusion on token embeddings produced embedding residuals rather than hidden states, yielding near-random acceptance (0.62\%). Switching to discrete mask-predict with the target's hidden states immediately improved accuracy.
  \item[v6 $\to$ v7 (RoPE regression):] Adding RoPE positional encoding \emph{degraded} performance from $\tau{=}0.43$ to $\tau{=}0.04$. The drafter already receives position information through the target's hidden states; explicit positional encoding created conflicting signals.
  \item[v6 $\to$ v3 (Single-shot training):] Training with single-shot loss ($\gamma{=}7.0$ exponential decay) plus deep KV injection produced a drafter that is robust across all masking ratios. This same checkpoint achieves $\tau{=}2.9$ in single-shot mode and $\tau{=}9.4$ with iterative denoising --- a $22{\times}$ improvement over the v6 discrete baseline.
\end{description}
